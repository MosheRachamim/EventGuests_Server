...
router.get('/getTablesStats', function (req, res) {
  ...
  var sql2 = "Select sum(num_guests) as approved,COALESCE(sum(new_num_guests),0) as arrived,COALESCE(sum(num_guests)/sum(new_num_guests)*100,0) as percentage from guests where event_id=" + event_id;
  var sql3 =
    "select table_number, sum(num_guests) as approved, COALESCE(sum(new_num_guests),0) as arrived, COALESCE(sum(num_guests) /sum(new_num_guests)*100,0) as percentage " +
    "from guests " +
